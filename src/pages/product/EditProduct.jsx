...
const EditProductPage = () => {
  ...
  const fetchProductFields = async () => {
    try {
      ...
      if (res.data && Array.isArray(res.data)) {
        const globalFields = res.data.map(f => f.name);
        setFormData(prev => {
          const updatedCustomFields = { ...prev.custom_field };
          globalFields.forEach(fieldName => {
            if (updatedCustomFields[fieldName] === undefined) {
              updatedCustomFields[fieldName] = "";
            }
          });
          return { ...prev, custom_field: updatedCustomFields };
        });
      }
    } catch (error) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      setFormData({
        ...
        necklace_thickness: product.necklace_thickness || "",
        ideal_for: product.ideal_for || "",
        number_of_gems: product.number_of_gems || "",
        visibility: product.visibility,
        is_custom: product.is_custom,  
        custom_field: product.custom_feilds,
        dealer_id: product.dealer_id || "",
        purchase_price: product.purchase_price || ""
      });
      setSelectedGroup(product.group_name);
      if (product.media) {
        const imageMedia = product.media.filter(media => 
          media.file_type.includes('image')
        );
        setExistingImages(imageMedia.map((media, index) => ({
          id: media.id,
          url: `${IMAGE_PATH}${media.file_path}`,
          order: index,
          isExisting: true
        })));
      }
      ...
      if (product.media) {
        const videoMedia = product.media.filter(media => 
          media.file_type.includes('video') || media.file_type === 'iframe'
        );
        
        if (videoMedia.length > 0) {
          const videoSections = videoMedia.map((video, index) => ({
            id: index + 1,
            useUrl: video.file_type === 'iframe',
            useFile: video.file_type.includes('video'),
            video_url: video.file_type === 'iframe' ? video.file_path : "",
            video_file: null,
            video_preview: video.file_type.includes('video') ? `${IMAGE_PATH}${video.file_path}` : "",
            existing_video: video,
            isExpanded: index === 0
          }));
          setVideos(videoSections);
        } else {
          setVideos([{
            id: 1,
            useUrl: true,
      ...
      if(product.varient_image_path){
        setVariantThumbnailPreview(`${IMAGE_PATH}${product.varient_image_path}`);
      }

      if (product.media) {
        const videoMedia = product.media.find(media => 
          media.file_type.includes('video')
        );
        if (videoMedia) {
          setExistingVideo(videoMedia);
        }
      }

      if (product.product_keywords && product.product_keywords.length > 0) {
        setSelectedKeywords(product.product_keywords.map(kw => kw.keyword));
      }

      setIsRecommended(product.recommended === 1);

      if (product.product_description && product.product_description.length > 0) {
        const sections = product.product_description.map(section => ({
          content: section.content || '',
          image: null,
          imagePreview: section.image_path ? IMAGE_PATH + section.image_path : null,
  ...
  const handleRemoveImage = (id, isExisting = false) => {
    if (isExisting) {
  ...
  const handleDragStart = (event) => {
    ...
    const activeItem = [...existingImages, ...images].find(img => img.id === event.active.id);
    if (activeItem) {
  ...
  const handleDragEnd = (event) => {
    ...
    if (over && active.id !== over.id) {
      const allImages = [...existingImages, ...images].sort((a, b) => (a.order || 0) - (b.order || 0));
      
      const oldIndex = allImages.findIndex(item => item.id === active.id);
      const newIndex = allImages.findIndex(item => item.id === over.id);
      
      if (oldIndex !== -1 && newIndex !== -1) {
        const newAllItems = arrayMove(allImages, oldIndex, newIndex);
        
        const updatedAllItems = newAllItems.map((item, idx) => ({
          ...item,
          order: idx
        }));

        setExistingImages(updatedAllItems.filter(item => item.isExisting));
        setImages(updatedAllItems.filter(item => !item.isExisting));
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    Object.entries(formData).forEach(([key, val]) => {
      ...
      if (val !== null && val !== undefined) {
        if (key === 'specification') {
          ...
          data.append('is_custom', val ? 1 : 0);  
        } else if (key === 'custom_field') {
          data.append('custom_feilds',JSON.stringify(val)); 
        } else {
          data.append(key, val);
        }
      }
    ...
    videos.forEach((video, index) => {
      if (video.useUrl && video.video_url) {
        ...
        data.append(`videos[${index}][use_url]`, '1');
      } else {
        data.append(`videos[${index}][use_url]`, '0');
      }
      
      if (video.useFile && video.video_file) {
        data.append(`videos[${index}][video_file]`, video.video_file);
        data.append(`videos[${index}][use_file]`, '1');
      } else {
        data.append(`videos[${index}][use_file]`, '0');
      }
      
      if (video.existing_video && !video.video_file && !video.video_url) {
        data.append(`videos[${index}][existing_id]`, video.existing_video.id);
      }
    });

    existingImages.forEach((img) => {
      data.append("existing_images[]", img.id);
      data.append(`existing_images_order[${img.id}]`, img.order);
    });

    images.forEach((img, index) => {
      data.append("images[]", img.file);
      data.append(`images_order[${index}]`, img.order);
    });

    if (videoOptions.useFile && videoFile) {
      data.append("video", videoFile);
    } else if (videoOptions.useUrl && formData.video_url) {
    ...
    descriptionSections.forEach((section, index) => {
      if (section.sectionId) {
        data.append(`description_sections[${index}][id]`, section.sectionId);
      }
      ...
      if (section.existingImage && !section.image) {
        data.append(`description_sections[${index}][existing_image]`, section.existingImage);
      }
      
      if (section.existingImage && !section.imagePreview && !section.image) {
        data.append(`description_sections[${index}][remove_image]`, 1);
      }
    });
  ...
  return (
    <div className="container py-2">
        ...
        <div className="card-body p-4">
          <form onSubmit={handleSubmit} className="needs-validation" noValidate>
            <div className="row g-4">
              ...
              <div className="col-md-6">
                <div className="mb-3">
                  ...
                  {(existingImages.length > 0 || images.length > 0) && (
                    ...
                    >
                      <SortableContext 
                        items={[...existingImages, ...images].sort((a,b) => (a.order || 0) - (b.order || 0)).map(img => img.id)} 
                          ...
                          {[...existingImages, ...images]
                            ...
                            .map((img, index) => (
                              <SortableImageItem
                                ...
                                handleRemoveImage={() => handleRemoveImage(img.id, img.isExisting)}
                              />
                            ))}
                        </div>
                      </SortableContext>
                  ...
                  {videos.map((video, index) => (
                    <div key={video.id} className="card border-0 shadow-sm mb-3">
                      <div className="card-body">
                        ...
                        <div className="mb-3">
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              updateVideoSection(video.id, "useFile", isFile);
                              updateVideoSection(video.id, "useUrl", !isFile);
                            }}
                          >
                        ...
                        {!video.useFile && (
                          <div className="form-floating mb-3">
                            <input
                              ...
                              onChange={(e) =>
                                updateVideoSection(video.id, "video_url", e.target.value)
                              }
                            />
                        ...
                        {video.useFile && (
                          <div className="mb-3">
                            <label className="btn btn-outline-primary w-100">
                              ...
                              {video.existing_video ? "Replace Video" : "Select Video"}
                              <input
                                type="file"
                                className="d-none"
                                accept="video/*"
                            ...
                            {(video.video_file || video.existing_video) && (
                              <div className="mt-2">
                                <button
                        ...
                        {/* PREVIEW */}
                        {(video.video_preview || video.existing_video) && (
                          <div className="mt-3">
                            ...
                            >
                              <source
                                src={
                                  video.video_preview ||
                                  (video.existing_video && `${IMAGE_PATH}${video.existing_video.file_path}`)
                                }
                              />
                            </video>
                          </div>
                        ...
                        {video.video_file && (
                          <small className="text-muted">
                            File: {video.video_file.name} (
                            {(video.video_file.size / (1024 * 1024)).toFixed(2)} MB)
                          </small>
                        )}
                      </div>
                    </div>
                  ...
                  {videos.length === 0 && (
                    <div className="text-center py-4 border rounded bg-light">
                      ...
                      <button 
                        ...
                        onClick={addVideoSection}
                      >
                        <i className="bi bi-plus-circle me-1"></i>Add First Video
                      </button>
                    </div>
                  )}
                </div>
              ...
              {/* ========== 7. ADDITIONAL PRODUCT ATTRIBUTES ========== */}
              <div className="col-12 mt-4">
                <h6 className="text-secondary mb-3 fw-semibold border-bottom pb-2">
              ...
              <div className="col-12">
                ...
                <div className="row g-4">
                  {Object.entries(formData.custom_field || {}).map(([key, value]) => (
                    <div className="col-md-4" key={key}>
                      <div className="position-relative">
                        ...
                        <div className="position-absolute end-0 top-0 me-2 mt-2 d-flex gap-1">
                          {/* Edit label */}
                          <button
                            type="button"
              ...
              <div className="col-12">
                {descriptionSections.map((section, index) => (
                  <DescriptionSection
                    key={`desc-${index}-${section.sectionId || 'new'}-${section.imagePreview ? 'with-image' : 'no-image'}`}
                    section={section}
                    index={index}
              ...
              <div className="col-12 mt-4 pt-2">
                <div className="d-flex justify-content-end gap-3">
                  ...
                  <button
                    ...
                    disabled={isSaving}
                  >
                    {!isSaving ? 'Update Product' : 'Saving..'}
                  </button>
                </div>
              </div>
      ...
      {(openCategoryDialog || openSizeDialog || openVariantDialog || customFieldModalOpen || editLabelModalOpen || showGroupingModal) && (
