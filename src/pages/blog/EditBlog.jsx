...
const BlogEdit = () => {
  ...
  const fetchBlog = async () => {
    try {
      ...
      if (blog.og_image) {
        setOgImagePreview(
          blog.og_image.startsWith("http")
            ? blog.og_image
            : `${IMAGE_PATH}${blog.og_image}`
        );
      }
      ...
      if (Array.isArray(blog.sections) && blog.sections.length > 0) {
        const mapped = blog.sections.map((section) => ({
          id: section.id,
  ...
  const handleThumbnailChange = (e) => {
    ...
    if (thumbnailPreview && !thumbnailPreview.startsWith(IMAGE_PATH)) {
  ...
  const handleCoverImageChange = (e) => {
    ...
    if (coverImagePreview && !coverImagePreview.startsWith(IMAGE_PATH)) {
  ...
  const handleOgImageChange = (e) => {
    ...
    if (ogImagePreview && !ogImagePreview.startsWith(IMAGE_PATH)) {
  ...
  const handleSectionImageChange = (index, e) => {
    ...
    setDescriptionSections((prev) => {
      ...
      if (
        ...
        !current.imagePreview.startsWith(IMAGE_PATH)
      ) {
  ...
  const removeDescriptionSection = (index) => {
    setDescriptionSections((prev) => {
      ...
      if (!current.id) {
        ...
        copy[index] = { ...current, isDeleted: true };
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const visibleSections = descriptionSections.filter(
        (s) => !s.isDeleted && (s.content || s.imageFile || s.imagePreview)
      );
      ...
      const sectionsToCreate = visibleSections.filter((s) => !s.id);
      const sectionsToUpdate = visibleSections.filter((s) => s.id);

      const sectionsToDelete = descriptionSections
        .filter((s) => s.id && s.isDeleted)
        .map((s) => s.id);

      // create
      sectionsToCreate.forEach((section, index) => {
        if (section.content && section.content !== "<p></p>") {
          data.append(
            `sections[create][${index}][content]`,
            section.content
          );
        }
        if (section.imageFile) {
          data.append(
            `sections[create][${index}][image]`,
            section.imageFile
          );
        }
        data.append(`sections[create][${index}][order]`, section._order ?? 0);
      });

      // update
      sectionsToUpdate.forEach((section, index) => {
        data.append(`sections[update][${index}][id]`, section.id);

        if (section.content && section.content !== "<p></p>") {
          data.append(
            `sections[update][${index}][content]`,
            section.content
          );
        } else {
          data.append(`sections[update][${index}][content]`, "");
        }

        if (section.imageFile) {
          data.append(
            `sections[update][${index}][image]`,
            section.imageFile
          );
        }

        data.append(`sections[update][${index}][order]`, section._order ?? 0);
      });

     
      sectionsToDelete.forEach((sectionId) => {
        data.append("sections[delete][]", sectionId);
      });

      await apiClient.post(`/blogs/${id}`, data, {
    ...
    return () => {
      ...
      descriptionSections.forEach((section) => {
        if (
          ...
          !section.imagePreview.startsWith(IMAGE_PATH)
        ) {
            ...
            <div className="row g-4">
              ...
              <div className="col-12 mt-4">
                ...
                <p className="text-muted small mb-3">
                  ...
                  Each block has rich content and an optional image. Recommended
                  image ratio: <strong>4 : 5</strong>.
                </p>
              </div>

              <div className="col-12">
                {descriptionSections.map((section, index) =>
                  section.isDeleted ? null : (
                    <div
                              ...
                              {!section.imagePreview ? (
                                ...
                                <>
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      handleRemoveSectionImage(index)
                                    }
                                  >
                                    Remove
                                  </button>
